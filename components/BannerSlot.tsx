...
export default function BannerSlot({ id = 'default-banner', className = '', type = 'inline' }: BannerSlotProps) {
  ...
  return (
    <div
      id={id}
      className={`relative overflow-hidden rounded-xl border border-dashed border-neutral-800 bg-neutral-900/30 flex items-center justify-center transition-all ${typeStyles[type]} ${className}`}
    >
      {/* 배경 장식 그라데이션 */}
      <div className="absolute inset-0 bg-gradient-to-r from-violet-500/10 via-fuchsia-500/10 to-cyan-500/10 blur-sm pointer-events-none" />
